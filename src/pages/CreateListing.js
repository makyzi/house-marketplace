...
const CreateListing = () => {
	...
	const onSubmit = async (e) => {
		...
		if (discountedPrice > regularPrice) {
			setLoading(false)
			toast.error('Discounted price should be lower then regular? ...')
			return
		}

		if (images.length > 6) {
			setLoading(false)
			toast.error('Can only upload up to 6 images per listing')
			return
		}
		...
		if (geolocationEnabled) {
			const response = await fetch(
				`https://maps.googleapis.com/maps/api/geocode/json?address=${address}&key=AIzaSyCs1yoLov3-aUfHDdW1MvWPyw7DrUvj_CI`
			)
